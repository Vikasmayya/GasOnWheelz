...
const NCOrders = () => {
    ...
    useEffect(() =>{
      try {
        const getPostFromFirebase = [];
          ...
          .where('order_type', '==','NEW_CONNECTION')
    ...
    return (
        <>
          ...
          <div className="my-2">
                <h4 className="text-center">Customer's INCOMPLETE Orders - New Connection</h4>
          </div>
          <div className="table-responsive" style={{padding:"20px"}}>
           ...
           {
               INBookings.map((post) => {
                 const { key, name, gas_cylinder_type, quantity, ncprice, booked_on, amount, amountr, pending_amount, phone, order_type} = post; 
               return(
                <tr key={key} style={{backgroundColor:"white" }}>
                    ...
                    <td>
                     <Link to={`/NCEditBooking/${phone}/${gas_cylinder_type}/${ncprice}/${name}/${key}/${quantity}/${amountr}/${order_type}`} className="btn btn-primary">
                          Edit
                     </Link>
                    </td>
                    </tr>
          ...
          <div className="my-2">
                <h4 className="text-center">Customer's COMPLETE Orders - New Connection</h4>
          </div>
          <div className="table-responsive" style={{padding:"20px"}}>
           <table className="table table-borderless">
           ...
           {
               Bookings.map((post) => {
                 const { key, gas_cylinder_type, quantity, ncprice, booked_on, amount, amountr, pending_amount } = post; 
               return(
                <tr key={key} style={{backgroundColor:"white" }}>
                    <td>{gas_cylinder_type}</td>
                    <td>{quantity}</td>
